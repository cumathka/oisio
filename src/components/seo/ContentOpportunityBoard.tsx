...
export function ContentOpportunityBoard({ onAskCopilot }: { onAskCopilot?: (prompt: string) => void }) {
  ...
  const getStageBadge = (stage: ContentCluster['stage']) => {
    switch (stage) {
      case 'TOFU':
        return <Badge variant="purple" size="sm">TOFU • Awareness</Badge>;
      case 'MOFU':
        return <Badge variant="info" size="sm">MOFU • Consideration</Badge>;
      case 'BOFU':
        return <Badge variant="success" size="sm">BOFU • Decision</Badge>;
    }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
            ...
            <Badge variant="ai" size="sm">Topic Authority Model</Badge>
          </h3>
          <p className="text-xs text-slate-400 mt-0.5">
            Target high-intent search gaps across Top-of-Funnel (TOFU) to Bottom-of-Funnel (BOFU).
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {clusters.map((c) => (
          <Card key={c.id} variant="default" padding="md" className="flex flex-col justify-between">
            <div className="space-y-3">
              ...
              <h4 className="text-xs font-semibold text-slate-200 leading-snug">{c.title}</h4>
              ...
              <div className="p-2.5 rounded-lg bg-slate-950/70 border border-slate-800/80 space-y-1.5 text-xs">
                ...
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="font-mono text-emerald-400">{c.volume} ({c.estTraffic})</span>
                </div>
              </div>
            </div>

            <div className="pt-3 border-t border-slate-800/80 mt-4 flex items-center justify-between">
              <span className="text-[10px] text-slate-500 font-medium">{c.impact}</span>
              <Button
                size="xs"
                variant="ai"
                onClick={() =>
                  onAskCopilot &&
                  onAskCopilot(`Draft a high-ranking content outline for: "${c.title}" targeting ${c.targetKeyword}`)
                }

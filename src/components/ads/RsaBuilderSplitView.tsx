...
export function RsaBuilderSplitView({ onAutoShorten }: RsaBuilderSplitViewProps) {
  ...
  // Auto-shorten action
  const handleAutoShorten = async (idx: number, type: 'headline' | 'description') => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <Card variant="surface" padding="md" className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-bold text-slate-100 flex items-center gap-2">
              ...
              <Badge variant="ai" size="sm">Deterministic Meter</Badge>
            </h3>
      ...
      <div className="lg:col-span-5 space-y-4">
        <Card variant="bordered" padding="md" className="space-y-4 sticky top-20">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h4 className="text-xs font-bold text-slate-200 flex items-center gap-1.5">
              <Eye className="w-3.5 h-3.5 text-indigo-400" />
              <span>Live Google Search Preview</span>
            </h4>
            <Badge variant="success" size="sm">Ad Strength: EXCELLENT</Badge>
          </div>
          ...
          <div className="p-4 rounded-xl bg-white text-slate-900 shadow-lg space-y-2 border border-slate-200 font-sans">
            ...
            <div className="flex items-center gap-1.5 text-[12px] text-slate-600">
              ...
              <span className="font-medium text-slate-800 truncate">
                https://example.ch &gt; {displayPath1} {displayPath2 && `> ${displayPath2}`}
              </span>
            </div>
            ...
            <div className="text-base font-semibold text-[#1a0dab] hover:underline cursor-pointer leading-snug">
              {headlines.filter((h) => h.trim().length > 0).slice(0, 3).join(' | ') ||
            ...
            <div className="pt-2 border-t border-slate-100 grid grid-cols-2 gap-2 text-xs text-[#1a0dab] font-medium">
              <span className="hover:underline cursor-pointer">Live Demo anfordern</span>
              <span className="hover:underline cursor-pointer">Preise & CHF Rechner</span>
            </div>
          </div>

          {/* Ad Strength Factors Check */}
          <div className="space-y-2 pt-2">
            <h5 className="text-xs font-semibold text-slate-300">Google RSA Quality Checklist</h5>
